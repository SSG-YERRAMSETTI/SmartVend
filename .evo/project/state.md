...
Baseline architecture audit and EVO onboarding.
...
Nothing. This checkpoint closes the baseline milestone.

## Blocked

1. **Seed Live discovery.** No provider access, no credentials, no captured
   payloads. Blocks gameplan phases P1 onward and is the critical path.
2. **Backend validation.** The backend cannot import: `sqlalchemy`, `alembic`,
   `passlib` and `jwt` are missing and no virtualenv is committed. No backend
   validation command can be configured honestly until this is resolved.
3. **Schema changes.** Alembic is not initialized, so there is no mechanism.
   Decision D1.
...
1. Obtain Cantaloupe / Seed Live access and capture real payloads per report
   type. Nothing downstream can be designed responsibly without this.
2. Settle schema authority and initialize Alembic with Ganesh, decision D1.
3. Agree the minimum test and CI baseline, decision D5.
4. Decide remediation for committed customer data and the exposed credential,
   decision D6.
5. Once P1 evidence and D1 exist, define canonical model v1 for the live path,
   designed against the ADR-0001 target architecture.
